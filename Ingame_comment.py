...
def run_game():
    ...
    bgmusic.play(-1, 0)
    # อาจเพิ่มเสียงเตือนสั้นๆเวลาสุ่มเลขใหม่ขึ้นมา ผู้เล่นจะได้รู้ตัวทัน (คำแนะนำจาก เป็ดพะโล้ v.2)
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == SHOW_NUMBER_EVENT and not game_over:
                ...
                pygame.time.set_timer(SHOW_NUMBER_EVENT, 8000) # ความเร็วในการสุ่มเลขใหม่อาจช้าหรือเร็วเกินไปสำหรับแต่ละคน เพิ่มมีให้เลือกโหมดช้า เร็ว เพื่อผู้เล่นจะได้เลือกแบบที่สนุกสำหรับเขา (คำแนะนำจาก เป็ดพะโล้ v.2)
                draw_count += 1
        ...
        if game_over:
            ...
            # เล่นเสียงหลังจบเกม
            if not bingo_winner_sound: # เมื่อเริ่มเล่นอีกครั้งหลังจากที่ชนะ เพลงชนะจะไม่หยุดแต่จะเล่นซ้อนกับเพลงพื้นหลังเลยไปจนกว่าเพลงนั้นจะจบเอง (ข้อสังเกตจาก เป็ดพะโล้ v.2)
                bgmusic.stop()
                omg.play()
                wowclap.play()
                bingo_sfx.play()
